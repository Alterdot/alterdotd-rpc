...
function innerRpc(request, callback) {
  ...
  const errorMessage = 'Alterdot JSON-RPC: ';
  ...
  const req = this.protocol.request(options, (res) => {
    ...
    res.on('end', () => {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        const exceededError = new Error(`Alterdot JSON-RPC: ${buf.toString('utf8')}`);
...
// For definitions of RPC calls, see various files in: https://github.com/Alterdot/Alterdot/tree/master/src
RpcClient.callspec = {
